        ...
        class BananaCoast3D {
            ...
            syncPlayers(players,event,now,localProfileId){
                ...
                for(const player of players){
                    ...
                    const alive=!event?.leaderboard||event.leaderboard.find((entry)=>entry.profileId===id)?.alive!==false;node.visible=alive;node.userData.sword.visible=Boolean(event?.combat&&alive);if(node.userData.sword.visible)node.userData.sword.rotation.z=-.58+Math.sin(now*.006+Number(player.x))*.06;
                }
